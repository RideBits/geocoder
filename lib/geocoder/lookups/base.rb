...
module Geocoder
  module Lookup
    ...
    class Base
      ...
      def uses_proxy?
        if configuration.send(:proxy).nil?
          proxy_name = "#{protocol}_proxy"
          !configuration.send(proxy_name).nil?
        else
          true
        end
      end
      ...
      #
      def http_client(can_use_proxy = true)
        if can_use_proxy
      ...
      #     if uses_proxy?
      #       RestClient.proxy = get_proxy_url
